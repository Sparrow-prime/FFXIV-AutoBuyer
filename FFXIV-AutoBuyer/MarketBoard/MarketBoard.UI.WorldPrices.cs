...
public partial class MarketBoardModule
{
    ...
    )
    {
        ...
        // 右键：请求跨服传送（经 Lifestream IPC，未安装时给出提示）
        if (!isCurrentWorld && ImGui.IsItemClicked(ImGuiMouseButton.Right))
            RequestWorldTravel(world.WorldID, world.WorldName);
        ...
        {
            // 本服（青蓝）：与「选中」同色系，保证无论当前查看哪个服务器，本服卡片都是青蓝
            bgColor = KnownColor.DeepSkyBlue.ToVector4().WithW
            (
                isHovered ?
                    0.30f :
        ...
        using (UIFont(0.8f).Push())
        {
            ...
            if (isCurrentWorld)
            {
                ...
                badgeColor = KnownColor.DeepSkyBlue.ToUInt();
            }
            else
            {
                switch (cardType)
    ...
    )
    {
        ...
        using (ImRaii.Group())
        {
            if (ImGui.Checkbox($"{checkboxLabel}###OnlyCurrentDC", ref config.OnlyCurrentDC))
                SaveConfig(config);
    ...
    private sealed class WorldPriceCardComponent : CardComponentBase
    {
        ...
        // 需求：去掉鼠标指向服务器价格时的「浮动」效果（悬停不再上移），
        // 悬停反馈仅保留边框/高亮变化。
        protected override float HoverFloatOffset => 0f;
        ...
        )
        {
            ...
            // 右键：请求跨服传送（经 Lifestream IPC，未安装时给出提示）
            if (!isCurrentWorld && ImGui.IsItemClicked(ImGuiMouseButton.Right))
                frame.Owner.RequestWorldTravel(world.WorldID, world.WorldName);
