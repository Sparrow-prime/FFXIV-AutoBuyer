...
public class FontManager : OmenServiceBase<FontManager>
{
    ...
    protected override void Init()
    {
        ...
        // 【AutoBuyer 本地改动】不再在后台枚举本机已安装字体。
        // 该能力仅服务于 OmenTools 的字体选择界面（本插件不提供），而枚举会递归扫描
        // C:\Windows\Fonts 并用 GDI+ 逐个加载字体文件，属纯无谓开销。
        // 需要时仍可通过 RegenerateInstalledFontsAsync() 手动触发。
        Task.Run(() => RebuildUIFontsAsync(), cancelSource.Token).ConfigureAwait(false);
    }
    ...
    private void RegenerateInstalledFonts()
    {
        ...
        {
            DLog.Warning("已取消获取本机安装字体");
        }
        catch (Exception ex)
        {
            DLog.Error("尝试获取本机安装字体时发生错误", ex);
        }
        finally
        {
            ...
            foreach (var error in errors)
                DLog.Error("尝试获取本机安装字体时出错", error);
        }
    }
